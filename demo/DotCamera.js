    ...
    frame(glsl, {time, canvasSize}) {
        ...
        for (let i=0; i<10; ++i) {
            points = glsl({...arg, field:field.edge, imgForce:imgForce.edge.linear, seed: Math.random()*124237, FP: `
                ...
                vec2 force = f.xy*10.0 + imgForce(p.xy).xy*20.0 * (dayMode?-1.0:1.0);
            ...
            glsl({...arg, points:points[0], Grid: points[0].size, Blend:'s+d', Clear:0, VP:`
                ...
                FP:`vec3(XY,1.)*exp(-dot(XY,XY)*vec3(4,4,8)),0`}, field)
        }
        // draw dots on screen
        glsl({...arg, points:points[0], Grid: points[0].size, Clear:dayMode?1:0, 
            Blend:'d*(1-sa)+s*sa', VP:` VPos.xy = (points(ID.xy).xy + XY*4.0/canvasSize)*2.0-1.0;`,
            FP:`vec3(!dayMode),exp(-dot(XY,XY)*3.0)`})
    }
}
