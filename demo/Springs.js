...
class Springs {
    ...
    reset() {
        ...
        for (let i=0; i<2; ++i)
        this.points = glsl({FP:`
            XY*rot2(-0.2)*0.5+vec2(0,0.0),0,(UV.x+UV.y)*4.0
        `}, {size:[16, 16], format:'rgba32f', story:2, tag:'points'});
    }

    frame(glsl, {time}) {
        const {points} = this;

        const next = glsl({time, ...this.params, points:points[0], prev:points[1], FP:`
            FOut = points(I);
        ...
        glsl({time, FP:`
            ...
            void fragment() {
                ...
                ivec2 i, D = ViewSize;
                float f0 = sin(p.w);
                for (i.y=-2; i.y<=2; ++i.y) if (I.y+i.y>=0 && I.y+i.y<D.y)
                for (i.x=-2; i.x<=2; ++i.x) if (I.x+i.x>=0 && I.x+i.x<D.x) {
                    if (i==ivec2(0)) continue;
                    vec4 p1 = Src(I+i);
                    float f1 = sin(p1.w);
                    float f = 1.0 + 0.05*0.5*(f0+f1);
                    acc += relax(p.xyz, p1.xyz, f*length(vec2(i)/vec2(D)));
                }
                FOut.xyz = acc.xyz/acc.w;
                if (FOut.y<-0.8) {
                    FOut.y = -0.8;
                    FOut.x = mix(FOut.x, p.x, 0.8);
                }
            }`}, next);
        ...
        glsl({points:next[0], Mesh:[w-1,h-1], Aspect:'y', 
        Blend: 'd*(1-sa)+s*sa',VP:`
        points(VID.xy).xy,0,1`,
        FP:`sqrt(isoline(UV.x*float(Mesh.x))+isoline(UV.y*float(Mesh.y)))`});

        glsl({points:next[0], Grid:points[0].size, Aspect:'y', 
        Blend: 'd*(1-sa)+s*sa',VP:`
        VOut.xy = points(ID.xy).xy + XY*0.015;`,
